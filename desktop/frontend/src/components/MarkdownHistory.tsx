...
export const MarkdownHistory = memo(function MarkdownHistory({
  ...
  const writeOffset = useTranscriptScrollOffsetWrite();
  ...
  useLayoutEffect(() => {
    ...
    if (delta === 0) return;
    // Single-writer rule (desktop/AGENTS.md): the compensation goes through
    // the arbiter's offset channel, never a raw scroller write. Without a
    // provider (no transcript surface) there is nobody to route to.
    writeOffset?.("block-window-prepend", pending.scroller.scrollTop + delta);
  }, [blockWindow.end, blockWindow.start, blocks, writeOffset]);
