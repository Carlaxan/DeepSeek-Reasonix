...
func New(cfg provider.Config) (provider.Provider, error) {
	...
	// DeepSeek's official Anthropic-compatible endpoint accepts image input
	// only on its declared vision model. Enforce that wire constraint here as
	// defense in depth, independent of config or extension capability metadata.
	vision = vision && (!officialDeepSeek || openai.IsDeepSeekVisionModel(cfg.Model))
