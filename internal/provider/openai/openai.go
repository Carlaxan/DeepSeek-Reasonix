...
func New(cfg provider.Config) (provider.Provider, error) {
	...
	// DeepSeek's official chat API accepts image input only on its declared
	// vision model. Keep this provider-boundary guard even though config
	// capability resolution normally prevents image attachments from reaching
	// this layer. No persisted or extension-supplied capability flag may
	// override the endpoint's wire contract for other models.
	vision = vision && (!officialDeepSeek || IsDeepSeekVisionModel(cfg.Model))
